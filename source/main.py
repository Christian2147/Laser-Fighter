...
# This is game version beta 1.2.2 released on 08/20/2025
...
import random
import gc
from setup.WindowSetup import GameWindow
from setup.TextureSetup import TextureSetup
...
from components.spawn.SpawnBackgroundObjects import SpawnGround
from components.spawn.SpawnBackgroundObjects import SpawnShip
...
def main():
    ...
    human_player = SpawnHumanPlayer(textures, window.scale_factor_X, window.scale_factor_Y)
    ...
    screen = ScreenUpdate(window, button, pop_up, settings, shop_config, refresh_variables,
    ...
    while running:
        ...
        for rm in red_machine.red_machines:
            if rm.machine_visible == 1:
                window.screen.blit(rm.image, rm.rect)

                if rm.red_machine_laser.laser_visible == 1:
                    window.screen.blit(rm.red_machine_laser.image, rm.red_machine_laser.rect)
        ...
        if screen.mode == "Machine_Mode":
                ...
                for rm in red_machine.red_machines:
                    ...
                    if rm.get_red_machine().isvisible() and red_machine.red_machines_update_values[current_red_update_value_index] == 0:
                        if rm.health_bar <= machine_mode_setup.damage and rm.hit_delay == 0:
                            ...
                            for i in range(len(p.get_laser())):
                                if p.get_laser()[i].isvisible() and \
                                 (rm.rect.centerx - 64 * window.scale_factor_X < p.get_laser()[i].rect.centerx < rm.rect.centerx + 64 * window.scale_factor_X) and \
                                 (rm.rect.centery - 64 * window.scale_factor_Y < p.get_laser()[i].rect.centery < rm.rect.centery + 64 * window.scale_factor_Y):
                                    attacked = 1
                            ...
                            if attacked == 1:
                                ...
                                red_machine.red_machines_update_values[current_red_update_value_index] = \
                                red_machine.red_machines_update_values[current_red_update_value_index] + 1
                    ...
                    if rm.get_red_machine().isvisible() and red_machine.red_machines_hit_values[current_red_hit_value_index] == 0:
                        if rm.health_bar > machine_mode_setup.damage:
                            ...
                            if attacked == 1:
                                ...
                                red_machine.red_machines_hit_values[current_red_hit_value_index] = \
                                red_machine.red_machines_hit_values[current_red_hit_value_index] + 1
            ...
            for p in machine_player.current_player:
                ...
                # If the death animation is not ongoing
                    ...
                    for rm in red_machine.red_machines:
                        if rm.get_red_machine_laser().distance(p.get_player()) < 125 * window.scale_factor:
                            if rm.get_red_machine_laser().isvisible() and -30 * window.scale_factor_X < (
                                    rm.get_red_machine_laser().rect.centerx - p.rect.centerx) < 30 * window.scale_factor_X:
                ...
                # If there is no hit delay
                    ...
                    for rm in red_machine.red_machines:
                        if rm.get_red_machine_laser().distance(p.get_player()) < 125 * window.scale_factor:
                            if rm.get_red_machine_laser().isvisible() and -30 * window.scale_factor_X < (rm.get_red_machine_laser().rect.centerx - p.rect.centerx) < 30 * window.scale_factor_X:
                                rm.set_laser_has_attacked(1)
                                if p.get_death_animation() == 0 and p.get_health_bar_indicator() != 1 and p.get_health_bar_indicator() != 0 and p.get_hit_delay() == 0 and settings.god_mode == 0:
                                    p.hit_player(settings.player_hit_sound)
                                    machine_player.player_hit_value = machine_player.player_hit_value + 1
        ...
        if screen.mode == "Alien_Mode":
                    ...
                    if medium_alien.medium_aliens_kill_values[current_medium_alien_update_value_index] == 0:
                        if ma.health <= alien_mode_setup.damage and ma.get_medium_alien().isvisible() and ma.hit_delay == 0:
                            ...
                            if ma.got_hit == 0:
                                for l in h.get_laser():
                                    if l.laser_update < alien_mode_setup.piercing and \
                                        alien_collision.MEDIUM_ALIEN_Y_RANGE[0] < l.rect.centery < alien_collision.MEDIUM_ALIEN_Y_RANGE[1] and (
                                        (l.rect.centerx > ma.rect.centerx + alien_collision.MEDIUM_ALIEN_X_DISTANCE * ma.collision_point and h.laser_direction == 1 and ma.already_ahead == 0) or
                                        (l.rect.centerx < ma.rect.centerx + alien_collision.MEDIUM_ALIEN_X_DISTANCE * ma.collision_point and h.laser_direction == 2 and ma.already_behind == 0)
                                    ):
                            ...
                            if ma.thorns_initiated_damage == 1 and laser_killed == 0:
                                medium_alien.medium_aliens_kill_values[current_medium_alien_update_value_index] = \
                                medium_alien.medium_aliens_kill_values[current_medium_alien_update_value_index] + 1
                        ...
                        ma.kill_alien(settings.enemy_death_sound, coin.coins_on_screen_list)
                        medium_alien.medium_aliens_kill_values[current_medium_alien_update_value_index] = \
                        medium_alien.medium_aliens_kill_values[current_medium_alien_update_value_index] + 1
                            ...
                            if ma.got_hit == 0:
                                for l in h.get_laser():
                                    if l.laser_update < alien_mode_setup.piercing and \
                                        ...
                                        medium_alien.medium_aliens_hit_values[current_medium_alien_hit_value_index] = \
                                        medium_alien.medium_aliens_hit_values[current_medium_alien_hit_value_index] + 1
            ...
            for h in human_player.current_human:
                ...
                # If the death animation is not ongoing
                    ...
                    for ma in medium_alien.medium_aliens:
                        if ma.get_medium_alien().distance(h.get_player()) < 100 * window.scale_factor:
                            if h.health == 1 and h.hit_delay == 0 and ma.rect.centerx - 15 * window.scale_factor_X < h.rect.centerx < ma.rect.centerx + 15 * window.scale_factor_X and human_player.human_update_value == 0 and ma.get_death_animation() == 0 and settings.god_mode == 0:
                                h.kill_player(settings.player_death_sound)
                                human_player.human_update_value = human_player.human_update_value + 1
                                if shop_config.thorns_enabled:
                ...
                # If there is no hit delay
                    ...
                    for ma in medium_alien.medium_aliens:
                        if ma.get_medium_alien().distance(h.get_player()) < 100 * window.scale_factor:
                            if h.get_health() > 1 and ma.rect.centerx - 15 * window.scale_factor_X < h.rect.centerx < ma.rect.centerx + 15 * window.scale_factor_X and ma.get_death_animation() == 0 and h.get_hit_delay() == 0 and settings.god_mode == 0:
                                h.hit_player(settings.player_hit_sound)
                                human_player.human_hit_value = human_player.human_hit_value + 1
                                if shop_config.thorns_enabled:
                                    ma.thorns_initiated_damage = 1

...
rem WARNING: Do NOT move or set the batch script to ANY other directory. This could cause unintended side effects!
rem I am not responsible for any damage caused my misuse of this file!
cd /d "%~dp0"

rem Check if the script is located in the textures folder (As a safety check to avoid unintended side effects)
for %%x in ("%~dp0.") do (
    if /I "%%~nxx" neq "textures" (
        echo ERROR: This script must be run from the "textures" directory.
        echo Current directory: %%~nxx
        pause
        exit /b 1
    )
)

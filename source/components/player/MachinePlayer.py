...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, god_mode, textures, scale_factor_x, scale_factor_y):
        ...
        self.laser_list = []
        self.laser_float_y_list = [0.0] * self.laser_count
        ...
        for i in range(machine_mode_setup.laser_count):
            ...
            self.laser_float_y_list[i] = float(self.rect.centery)
    ...
    def __del__(self):
        ...
        for laser in self.laser_list:
            if hasattr(laser, 'kill'):
                laser.kill()
            del laser

        if hasattr(self.health_bar, 'kill'):
            self.health_bar.kill()
        del self.health_bar

        self.laser_list.clear()
        self.laser_float_y_list.clear()
        self.laser_start_y_list.clear()
        self.laser_has_attacked_list.clear()
        self.lasers_fired_list.clear()

        if hasattr(self, 'armor_bar'):
            if hasattr(self.armor_bar, 'kill'):
                self.armor_bar.kill()
            del self.armor_bar
    ...
    def remove(self):
        ...
        self.laser_count = 0
        self.do_collision = 0
        self.health_bar.health_bar_visible = 0
    ...
    def move_player(self, yellow_power_up):
        ...
        if self.direction == 1 and self.rect.centerx > 20 * self.scale_factor_x and self.death_animation == 0:
            # How fast the player moves is determined by whether or not the yellow power up is active
            if yellow_power_up == 1:
                self.rect.centerx = self.rect.centerx - machine_mode_setup.yellow_player_movement
            else:
                self.rect.centerx = self.rect.centerx - machine_mode_setup.player_movement

        if self.direction == 2 and self.rect.centerx < 1260 * self.scale_factor_x and self.death_animation == 0:
            if yellow_power_up == 1:
                self.rect.centerx = self.rect.centerx + machine_mode_setup.yellow_player_movement
            else:
                self.rect.centerx = self.rect.centerx + machine_mode_setup.player_movement
    ...
    def fire(self, shooting_sound, index=0):
        ...
        self.laser_list[index].laser.laser_visible = 1
        if shooting_sound == 1:
    ...
    def shoot(self, shooting_sound, yellow_power_up):
        ...
        for i in range(len(self.laser_has_attacked_list)):
            if self.laser_has_attacked_list[i] == 1:
                self.laser_list[i].laser.laser_visible = 0

        # Fire the third laser when the second laser has travelled at least 100 units
        if len(self.laser_list) > 2 and self.lasers_fired_list[1] != 0 and self.lasers_fired_list[2] == 0 and self.laser_list[1].laser.rect.centery <= self.laser_start_y_list[1] - 100 * self.scale_factor_y:
            self.fire(shooting_sound, 2)

        # Fire the second laser when the first laser has travelled at least 100 units
        if len(self.laser_list) > 1 and self.lasers_fired_list[1] == 0 and self.laser_list[0].laser.rect.centery <= self.laser_start_y_list[0] - 100 * self.scale_factor_y:
            ...
            if elapsed_time >= 0.015:
                ...
                if yellow_power_up == 0:
                    ...
                    if len(self.lasers_fired_list) == 3 and self.lasers_fired_list[2] == 1:
                        self.laser_list[0].laser.rect.centery = self.laser_list[0].laser.rect.centery - machine_mode_setup.laser_speed - delta_movement
                        self.laser_list[1].laser.rect.centery = self.laser_list[1].laser.rect.centery - machine_mode_setup.laser_speed - delta_movement
                        self.laser_list[2].laser.rect.centery = self.laser_list[2].laser.rect.centery - machine_mode_setup.laser_speed - delta_movement
                    ...
                    if len(self.lasers_fired_list) == 3 and self.lasers_fired_list[2] == 1:
                        self.laser_list[0].laser.rect.centery = self.laser_list[0].laser.rect.centery - machine_mode_setup.yellow_power_up_speed - delta_movement
                        self.laser_list[1].laser.rect.centery = self.laser_list[1].laser.rect.centery - machine_mode_setup.yellow_power_up_speed - delta_movement
                        self.laser_list[2].laser.rect.centery = self.laser_list[2].laser.rect.centery - machine_mode_setup.yellow_power_up_speed - delta_movement
                    elif len(self.lasers_fired_list) >= 2 and self.lasers_fired_list[1] == 1:
                        self.laser_list[0].laser.rect.centery = self.laser_list[0].laser.rect.centery - machine_mode_setup.yellow_power_up_speed - delta_movement
                        self.laser_list[1].laser.rect.centery = self.laser_list[1].laser.rect.centery - machine_mode_setup.yellow_power_up_speed - delta_movement
                    elif len(self.lasers_fired_list) >= 1 or self.lasers_fired_list[1] == 0:
